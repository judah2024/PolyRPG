...
namespace RPGCharacterAnims
{
    public class RPGCharacterWeaponController : MonoBehaviour
    {
        ...
        public void UnsheathWeapon(Lookups.Weapon weapon, bool dual)
        {
        ...
        private IEnumerator _UnSheathWeapon(Lookups.Weapon weapon)
        {
			if (!dualSwitch) {
			...
			var currentWeaponType = (Lookups.Weapon) animator.GetInteger(AnimationParameters.Weapon);
            ...
            if (weapon == Lookups.Weapon.Unarmed) {
				if (debugWalkthrough) { Debug.Log("Switching to Unarmed from Relax."); }
                DoWeaponSwitch(( int )AnimatorWeapon.RELAX, Lookups.Weapon.Relax, AnimatorWeapon.RELAX, Side.None, false);

				// Wait for WeaponSwitch() to happen then update Animator.
				yield return new WaitForSeconds(0.1f);
				SetWeaponWithDebug(AnimatorWeapon.UNARMED, -2, Lookups.Weapon.Unarmed, Lookups.Weapon.Unarmed, Side.None);
            }
            ...
            // Switching to 2Handed weapon.
				...
				if (currentWeaponType.Is2HandedWeapon()) {
					...
					SetWeaponWithDebug(weapon.ToAnimatorWeapon(), -2, currentWeaponType, Lookups.Weapon.Unarmed, Side.Unchanged);
				}
				else {
					...
					// Wait for WeaponSwitch() to happen then update Animator.
					yield return new WaitForSeconds(0.75f);
					SetWeaponWithDebug(weapon.ToAnimatorWeapon(), -2, weapon, Lookups.Weapon.Unarmed, Side.Unchanged);
				}
			}
            ...
            // Switching to 1Handed weapon.
                ...
                if (weapon.IsLeftHandedWeapon()) {
					...
					if (weapon == Lookups.Weapon.Shield) {
		...
		/// <returns>IEnumerator for use with StartCoroutine.</returns>
		public IEnumerator _DualUnSheath(Lookups.Weapon weapon)
		{
			...
			yield return new WaitForSeconds(0.55f);
			SetWeaponWithDebug(AnimatorWeapon.ARMED, ( int )AnimatorWeapon.ARMED, (Lookups.Weapon)weapon -1, weapon, Side.Dual);
		}
		...
		public void SheathWeapon(Lookups.Weapon fromWeapon, Lookups.Weapon toWeapon, bool dual)
        {
        ...
        /// <returns>IEnumerator for use with StartCoroutine.</returns>
        public IEnumerator _SheathWeapon(Lookups.Weapon weaponToSheath, Lookups.Weapon weaponToUnsheath)
        {
			if (!dualSwitch) {
            ...
            if (weaponToUnsheath.HasNoWeapon()) {
				...
				if (rpgCharacterController.rightWeapon.HasEquippedWeapon()
                    ...
                    if (weaponToSheath.Is1HandedWeapon()) {
						...
						if (rpgCharacterController.hasDualWeapons) {
							...
							{ SetWeaponWithDebug(AnimatorWeapon.ARMED, -2, Lookups.Weapon.Unarmed, rpgCharacterController.rightWeapon, Side.Right); }

							// Set Right Weapon.
							else if (weaponToSheath.IsRightHandedWeapon())
							{ SetWeaponWithDebug(AnimatorWeapon.ARMED, -2, rpgCharacterController.leftWeapon, Lookups.Weapon.Unarmed, Side.Left); }
						}
						// Just switching 1 weapon.
						else { SetWeaponWithDebug(weaponToUnsheath.ToAnimatorWeapon(), -2, Lookups.Weapon.Unarmed, Lookups.Weapon.Unarmed, Side.None); }
					}
                    // Sheath 2Handed weapon.
						...
						// Wait for WeaponSwitch() to happen then update Animator.
						yield return new WaitForSeconds(0.55f);
						SetWeaponWithDebug(weaponToUnsheath.ToAnimatorWeapon(), -2, Lookups.Weapon.Unarmed, Lookups.Weapon.Unarmed, Side.Unchanged);
					}
				}
                // Unarmed, switching to Relax.
					...
					// Wait for WeaponSwitch() to happen then update Animator.
					yield return new WaitForSeconds(0.1f);
					SetWeaponWithDebug(AnimatorWeapon.RELAX, -2, Lookups.Weapon.Relax, Lookups.Weapon.Relax, Side.None);
				}
			}
            // Switching to 2Handed Weapon.
				...
				if (currentWeaponType.Is1HandedWeapon()) {
					...
					// Wait for WeaponSwitch() to happen then update Animator.
					yield return new WaitForSeconds(0.55f);
					SetWeaponWithDebug(AnimatorWeapon.UNARMED, -2, Lookups.Weapon.Unarmed, Lookups.Weapon.Unarmed, Side.Unchanged);
                }
            ...
            // Switching to 1Handed weapons.
				...
				if (currentWeaponType.Is1HandedWeapon()) {
					...
					yield return new WaitForSeconds(0.55f);
					if (weaponToSheath.IsLeftHandedWeapon())
					{ SetWeaponWithDebug(AnimatorWeapon.ARMED, ( int)AnimatorWeapon.ARMED, Lookups.Weapon.Unarmed, rpgCharacterController.rightWeapon, Side.Unchanged); }
					else if (weaponToSheath.IsRightHandedWeapon())
					{ SetWeaponWithDebug(AnimatorWeapon.ARMED, ( int )AnimatorWeapon.ARMED, rpgCharacterController.leftWeapon, Lookups.Weapon.Unarmed, Side.Unchanged); }
				}
			}
        }
		...
		/// <returns>IEnumerator for use with StartCoroutine.</returns>
		public IEnumerator _DualSheath(Lookups.Weapon weaponToSheath, Lookups.Weapon weaponToUnsheath)
		{
			...
			if (weaponToSheath == Lookups.Weapon.Unarmed && weaponToUnsheath == Lookups.Weapon.Relax) {
				if (debugWalkthrough) { Debug.Log("Switching to Relax from Unarmed."); }
				DoWeaponSwitch(-1, Lookups.Weapon.Relax, AnimatorWeapon.RELAX, Side.Unchanged, true);

				// Wait for WeaponSwitch() to happen then update Animator.
				yield return new WaitForSeconds(0.55f);
				SetWeaponWithDebug(AnimatorWeapon.RELAX, (int)AnimatorWeapon.RELAX, Lookups.Weapon.Unarmed, Lookups.Weapon.Unarmed, Side.None);
			}
			// Sheath 1Handed weapons.
				...
				// Switching to other 1Handed weapons.
					...
					yield return new WaitForSeconds(0.55f);
					StartCoroutine(_HideAllWeapons(false, false));
					SetWeaponWithDebug(AnimatorWeapon.UNARMED, ( int )AnimatorWeapon.UNARMED, Lookups.Weapon.Unarmed, Lookups.Weapon.Unarmed, Side.Dual);
				}
				// Switching to Unarmed/Relax.
					...
					yield return new WaitForSeconds(0.55f);
					StartCoroutine(_HideAllWeapons(false, false));
					SetWeaponWithDebug(weaponToUnsheath.ToAnimatorWeapon(), -2, Lookups.Weapon.Unarmed, Lookups.Weapon.Unarmed, Side.None);
				}
			}
		}
		...
		public void SheathAndUnSheath(Lookups.Weapon weaponFrom, Lookups.Weapon weaponTo, Side side)
		{ StartCoroutine(_SheathAndUnSheath(weaponFrom, weaponTo, side)); }

		private IEnumerator _SheathAndUnSheath(Lookups.Weapon weaponFrom, Lookups.Weapon weaponTo, Side side)
		{
		...
		/// <param name="weaponTo">Weapon switching to.</param>
		/// <param name="side">Weapon side.</param>
		public void DualSheathAndUnSheath(Lookups.Weapon weaponFrom, Lookups.Weapon weaponTo, Side side)
		{ StartCoroutine(_DualSheathAndUnSheath(weaponFrom, weaponTo, side)); }

		private IEnumerator _DualSheathAndUnSheath(Lookups.Weapon weaponFrom, Lookups.Weapon weaponTo, Side side)
		{
        ...
        public void InstantWeaponSwitch(Lookups.Weapon weapon)
		...
		/// <returns>IEnumerator for use with StartCoroutine.</returns>
		/// /// <summary>
		public IEnumerator _InstantWeaponSwitch(Lookups.Weapon weapon)
		{
			...
			// 2Handed.
				...
				{ rpgCharacterController.SetIKOn(( Lookups.Weapon )animator.GetInteger(AnimationParameters.Weapon)); }
			}
			// Switching to Unarmed or Relax which map directly.
			else {
				if (weapon == Lookups.Weapon.Unarmed) {
					rpgCharacterController.rightWeapon = Lookups.Weapon.Unarmed;
					rpgCharacterController.leftWeapon = Lookups.Weapon.Unarmed;
					animator.SetInteger(AnimationParameters.Weapon, 0);
				}
				else {
					rpgCharacterController.rightWeapon = Lookups.Weapon.Relax;
					rpgCharacterController.leftWeapon = Lookups.Weapon.Relax;
		...
		private void DoWeaponSwitch(int weaponSwitch, Lookups.Weapon weapon, AnimatorWeapon animatorWeapon, Side side, bool sheath)
		{
		...
		private void SetWeaponWithDebug(AnimatorWeapon animatorWeapon, int weaponSwitch, Lookups.Weapon leftWeapon, Lookups.Weapon rightWeapon, Side weaponSide)
		{
        ...
        public IEnumerator _WeaponVisibility(Lookups.Weapon weaponNumber, bool visible, bool dual)
        {
            ...
            var weaponType = (Lookups.Weapon)weaponNumber;
			switch (weaponType) {
				case Lookups.Weapon.TwoHandSword: SafeSetVisibility(twoHandSword, visible); break;
				case Lookups.Weapon.TwoHandSpear: SafeSetVisibility(twoHandSpear, visible); break;
				case Lookups.Weapon.TwoHandAxe: SafeSetVisibility(twoHandAxe, visible); break;
				case Lookups.Weapon.TwoHandBow: SafeSetVisibility(twoHandBow, visible); break;
				case Lookups.Weapon.TwoHandCrossbow: SafeSetVisibility(twoHandCrossbow, visible); break;
				case Lookups.Weapon.TwoHandStaff: SafeSetVisibility(staff, visible); break;
				case Lookups.Weapon.Shield: SafeSetVisibility(shield, visible); break;
				case Lookups.Weapon.Rifle: SafeSetVisibility(rifle, visible); break;
				case Lookups.Weapon.RightSpear: SafeSetVisibility(spear, visible); break;
        ...
        private IEnumerator _SyncWeaponVisibility()
        {
            ...
            var leftWeaponType = (Lookups.Weapon)rpgCharacterController.leftWeapon;
            switch (leftWeaponType) {
                case Lookups.Weapon.Shield: SafeSetVisibility(shield, true); break;
                case Lookups.Weapon.LeftSword: SafeSetVisibility(swordL, true); break;
                case Lookups.Weapon.LeftMace: SafeSetVisibility(maceL, true); break;
                case Lookups.Weapon.LeftDagger: SafeSetVisibility(daggerL, true); break;
                case Lookups.Weapon.LeftItem: SafeSetVisibility(itemL, true); break;
                case Lookups.Weapon.LeftPistol: SafeSetVisibility(pistolL, true); break;
            }

            var rightWeaponType = (Lookups.Weapon)rpgCharacterController.rightWeapon;
            switch (rightWeaponType) {
                case Lookups.Weapon.TwoHandSword: SafeSetVisibility(twoHandSword, true); break;
                case Lookups.Weapon.TwoHandSpear: SafeSetVisibility(twoHandSpear, true); break;
                case Lookups.Weapon.TwoHandAxe: SafeSetVisibility(twoHandAxe, true); break;
                case Lookups.Weapon.TwoHandBow: SafeSetVisibility(twoHandBow, true); break;
                case Lookups.Weapon.TwoHandCrossbow: SafeSetVisibility(twoHandCrossbow, true); break;
                case Lookups.Weapon.TwoHandStaff: SafeSetVisibility(staff, true); break;
                case Lookups.Weapon.RightSword: SafeSetVisibility(swordR, true); break;
                case Lookups.Weapon.RightMace: SafeSetVisibility(maceR, true); break;
                case Lookups.Weapon.RightDagger: SafeSetVisibility(daggerR, true); break;
                case Lookups.Weapon.RightItem: SafeSetVisibility(itemR, true); break;
                case Lookups.Weapon.RightPistol: SafeSetVisibility(pistolR, true); break;
                case Lookups.Weapon.Rifle: SafeSetVisibility(rifle, true); break;
                case Lookups.Weapon.RightSpear: SafeSetVisibility(spear, true); break;
            }
        }
    }
}