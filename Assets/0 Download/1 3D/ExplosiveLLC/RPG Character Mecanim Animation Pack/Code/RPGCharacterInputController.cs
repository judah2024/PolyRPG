	...
	public class RPGCharacterInputController : MonoBehaviour
    {
        ...
        private void Aiming()
        {
            if (rpgCharacterController.hasAimedWeapon) {
                ...
                if (rpgCharacterController.rightWeapon == Lookups.Weapon.TwoHandBow) {
		...
		private void SwitchWeapons()
		{
			...
			if (inputShield) {
				...
				context.side = "Left";
				context.type = "Switch";
				context.leftWeapon = Lookups.Weapon.Shield;
			...
			if (Mathf.Abs(inputSwitchUpDown) > 0.1f) {
				var twoHandedWeapons = new Lookups.Weapon[] {
					Lookups.Weapon.TwoHandSword,
					 Lookups.Weapon.TwoHandSpear,
					 Lookups.Weapon.TwoHandAxe,
					 Lookups.Weapon.TwoHandBow,
					 Lookups.Weapon.TwoHandCrossbow,
					 Lookups.Weapon.TwoHandStaff,
					 Lookups.Weapon.Rifle,
				};
			...
			if (inputSwitchLeftRight > 0.1f || inputSwitchLeftRight < -0.1f) {
				...
				if (inputSwitchLeftRight < -0.1f) {
					...
					{ weaponNumber = Lookups.Weapon.LeftSword; }
				...
				// Right-handed weapons.
					...
					{ weaponNumber = Lookups.Weapon.Unarmed; }
