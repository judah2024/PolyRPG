...
namespace RPGCharacterAnims.Actions
{
	public class SwitchWeaponContext
    {
        ...
        public Lookups.Weapon rightWeapon;
        public Lookups.Weapon leftWeapon;
        ...
        public SwitchWeaponContext()
        {
            ...
            this.rightWeapon = Lookups.Weapon.Relax;
            this.leftWeapon = Lookups.Weapon.Relax;
        }

        public SwitchWeaponContext(string type, string side, string sheathLocation = "Back", Lookups.Weapon rightWeapon = Lookups.Weapon.Relax, Lookups.Weapon leftWeapon = Lookups.Weapon.Relax)
        {
            this.type = type;
            this.side = side;
    ...
    public class SwitchWeapon : BaseActionHandler<SwitchWeaponContext>
    {
        ...
        protected override void _StartAction(RPGCharacterController controller, SwitchWeaponContext context)
		{
			...
			Lookups.Weapon fromRightWeapon = controller.rightWeapon;
			Lookups.Weapon toRightWeapon = context.rightWeapon;

			bool changeLeft = false;
			bool sheathLeft = false;
			bool unsheathLeft = false;
			Lookups.Weapon fromLeftWeapon = controller.leftWeapon;
			Lookups.Weapon toLeftWeapon = context.leftWeapon;
			...
			switch (context.side) {
				...
				case "right":
					...
					if (toRightWeapon.Is2HandedWeapon() && !fromLeftWeapon.HasNoWeapon()) {
						changeLeft = true;
						toLeftWeapon = Lookups.Weapon.Unarmed;
				...
				case "left":
					...
					if (fromRightWeapon.Is2HandedWeapon()) {
						changeRight = true;
						toRightWeapon = Lookups.Weapon.Unarmed;
					}
					break;
				case "dual":
					changeLeft = true;
			...
			if (context.type == "switch") {
				...
				if ((toAnimatorWeapon == AnimatorWeapon.ARMED && toLeftWeapon != Lookups.Weapon.Shield
					&& fromLeftWeapon != Lookups.Weapon.Shield && weaponController.singleAnimSwitch)
				...
				// Dual sheathing and unsheathing 2 weapons, only play 1 weapon switch animation.
					...
					&& toLeftWeapon != Lookups.Weapon.Shield && weaponController.singleAnimSwitchDual) {
			...
			if (context.type == "unsheath" || context.type == "switch") {
				...
				if ((controller.isRelaxed || controller.hasNoWeapon)
					&& (toLeftWeapon == Lookups.Weapon.Unarmed || toRightWeapon == Lookups.Weapon.Unarmed)) {
					fromLeftWeapon = Lookups.Weapon.Relax;
					fromRightWeapon = Lookups.Weapon.Relax;
					sheathLeft = false;
					sheathRight = false;
					unsheathLeft = false;
					unsheathRight = true;
			...
			if (context.type == "instant") {
				...
				if (toLeftWeapon == Lookups.Weapon.Unarmed || toLeftWeapon == Lookups.Weapon.Relax) {
			...
			// Non-instant weapon switch.
				...
				if (dualSheath && (sheathRight || sheathLeft)) {
					...
					weaponController.SheathWeapon(fromRightWeapon, Lookups.Weapon.Unarmed, dualSheath);
				}
				else {
					if (sheathLeft) {
		...
		private void EndSwitch(RPGCharacterController controller, RPGCharacterWeaponController weaponController, bool changeRight, Lookups.Weapon toRightWeapon, bool changeLeft, Lookups.Weapon toLeftWeapon)
		{
		...
		private static void DebugSwitchWeapon(RPGCharacterWeaponController weaponController, SwitchWeaponContext context, bool changeRight, bool changeLeft, bool sheathRight,
			bool sheathLeft, bool unsheathRight, bool unsheathLeft, Lookups.Weapon fromRightWeapon, Lookups.Weapon toRightWeapon, Lookups.Weapon fromLeftWeapon,
			Lookups.Weapon toLeftWeapon, bool dualWielding, bool dualUnsheath, bool dualSheath)
		{
